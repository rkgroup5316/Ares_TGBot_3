...
async def Chat_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if  len(UsersId) > 30:
        await update.message.reply_text("Maximum of 30 users can be only shown in one time.\n\nUsᴀɢᴇ: /chat_info (ᴄʜᴀᴛ_ɪᴅ)")
        return
    ...
    for chat_id_str in UsersId:
        try:
            ...
            try:
                ...
                await msg.edit_text(f"Cʜᴀᴛ ID {chat_id}: Bᴀᴅ ʀᴇᴏ̨ᴜᴇsᴛ. Eʀʀᴏʀ: {e.message}")
            except Exception as e:
                await msg.edit_text(f"Cʜᴀᴛ ID {chat_id}: Fᴀɪʟᴇᴅ ᴛᴏ ɢᴇᴛ ᴄʜᴀᴛ ɪɴғᴏʀᴍᴀᴛɪᴏɴ. Eʀʀᴏʀ: {e}")
        except ValueError:
