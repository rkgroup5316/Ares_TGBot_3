...
async def paste(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if replied_message:
        ...
        elif replied_message.document and replied_message.document.mime_type in [
            "text/plain",
            "application/json",
        ]:
    ...
    await paste_reply.edit_text(
        f"{output}", disable_web_page_preview=True, reply_markup=keyboard
    )
...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await pong_reply.edit_text(
        f"♔ <b>Ping Time:</b><i>{pong}</i>ms |♔  <b>Bot is alive since:</b> <i>{botuptime}</i>",
        parse_mode="HTML",
    )
