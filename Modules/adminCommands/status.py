...
from utils.helper.functions import get_readable_bytes, get_readable_time
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def draw_progressbar(coordinate, progress):
        try:
            ...
            draw.ellipse(
                (progress - 7, coordinate - 25, progress + 15, coordinate),
                fill="#FFFFFF",
            )
    ...
    msg = await update.message.reply_photo(
        photo=load_asset(r"Ares_TGBot_3\assets\progress.jpg"),
        caption=caption,
        parse_mode="HTML",
    )
    end = datetime.now()
...
async def dbstats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        stats_string, parse_mode="HTML", reply_markup=CLOSE_BUTTON
    )
...
async def LOG(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await update.message.reply_document(
            open("logs.txt", "rb"), caption="logs.txt", reply_markup=CLOSE_BUTTON
        )
    except Exception as error:
        await update.message.reply_text(
            f"An error occured while getting log file{error}", reply_markup=CLOSE_BUTTON
        )


STATS_CMD = CommandHandler(("stats", "health", "hh"), stats)
SPEED_CMD = CommandHandler(("speed", "speedtest", "net_speed"), Speed)
DBSTATS = CommandHandler(
    ("dbstats", "data_base_stats", "dataBaseStats", "dataBase_stats"), dbstats
)
