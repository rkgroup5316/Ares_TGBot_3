...
from Modules.chat_handller import (Chat_Info_command, Reply_handller,
                                   changeprompt_command, clear_history_commamd,
                                   media_handler, process_message)
from utils.decoders_ import rate_limit, restricted

logger.info("Importing commands handllers...")
from Modules.adminCommands.broad_cast import (WARN_USER_BROADCAST,
                                              Global_BROADCAST,
                                              Specific_BROADCAST)

logger.info("Importing admin Status commamds...")
from Modules.adminCommands.Admin_cmds import (BAN_CMD, CHAT_DATA_CMD,
                                              CHAT_INFO_CMD, UN_BAN_CMD)
from Modules.adminCommands.status import DBSTATS, LOG_CMD, SPEED_CMD, STATS_CMD

logger.info("Importing users util commands...")
from Modules.users_command.Utils import ID_CMD, PASTE_CMD, PING_CMD

logger.info("Importing users Commands....")
from Modules.users_command.google import (GOOGLE_SEARCH_COMMAND,
                                          GOOGLE_SERACH_IMG_COMMAND,
                                          WIKI_COMMAND, YT_COMMND)

logger.info("Importing Owners commands..")
from Modules.adminCommands.owner import (ADMINS_LIST_CMD,
                                         BOT_ACTIVATION_MESSAGE, OFF_COMMAD,
                                         REFRESH_CMD, ADD_admin_CMD,
                                         RM_admin_CMD)
from Modules.adminCommands.terminal import EXECUTE_COMMAND, SHELL_CMD
...
service = KeepAliveService(
    ping_interval=60, log_level=30  # Ping every 1 minutes  # warning
)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.effective_chat.type == "private":
        ...
        await update.effective_message.reply_photo(
            photo=asset,
            caption=PM_MESSAGE.format(
                first_name, OWNER_INFO_HTML, SUPPORT_CHAT_INFO_HTML
            ),
...
def main() -> None:
    ...
    mediaChat_handler = MessageHandler(
        filters.VOICE
        | filters.AUDIO
        | filters.VIDEO
        | filters.PHOTO
        | filters.Document.ALL & ~filters.Entity("MENTION"),
        media_handler,
    )
